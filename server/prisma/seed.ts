...
async function main() {
  ...
  const ownerRole = await prisma.role.create({
    data: { key: 'owner', name: 'Owner', description: 'Full organization control', isSystemRole: true },
  });
  const adminRole = await prisma.role.create({
    data: { key: 'admin', name: 'Admin', description: 'Administrative access', isSystemRole: true },
  });
  const memberRole = await prisma.role.create({
    data: { key: 'member', name: 'Member', description: 'Standard access', isSystemRole: true },
  });
  ...
  const permKeys = [
    ...
    'eyes.read', 'eyes.manage',
    'providers.read', 'providers.select',
    'integrations.read', 'integrations.connect',
    'dashboard.read'
  ];

  const permissions = await Promise.all(
    permKeys.map(key => prisma.permission.create({
      data: { key, name: key.split('.').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '), description: `Ability to ${key.replace('.', ' ')}` }
    }))
  );
  ...
  const adminExcluded = ['organization.delete', 'billing.manage', 'roles.delete'];
  for (const p of permissions) {
  ...
  const memberIncluded = ['dashboard.read', 'eyes.read', 'providers.read', 'integrations.read'];
  for (const p of permissions) {
    if (memberIncluded.includes(p.key)) {
      await prisma.rolePermission.create({ data: { roleId: memberRole.id, permissionId: p.id } });
    }
  }

  // 4. Seed Eye Types
  console.log('Seeding eye types...');
  const chatEye = await prisma.eyeType.create({ data: { key: 'chat', name: 'Chat Eye', description: 'Monitors the chat platform for read messages, threads, and communications.' } });
  const meetingEye = await prisma.eyeType.create({ data: { key: 'meeting', name: 'Meeting Eye', description: 'Monitors meeting transcriptions and related discussions.' } });
  const taskEye = await prisma.eyeType.create({ data: { key: 'task', name: 'Task Eye', description: 'Monitors project management platforms like Jira to track tickets, tasks, and sprints.' } });
  const codingEye = await prisma.eyeType.create({ data: { key: 'coding', name: 'Coding Eye', description: 'Monitors the coding platform for commits, messages, comments, and PR descriptions.' } });

  // 5. Seed Providers
  console.log('Seeding providers...');
  const slack = await prisma.provider.create({ data: { key: 'slack', name: 'Slack', oauthSupported: true } });
  const msTeams = await prisma.provider.create({ data: { key: 'microsoft_teams', name: 'Microsoft Teams', oauthSupported: true, isActive: false } });
  const discord = await prisma.provider.create({ data: { key: 'discord', name: 'Discord', oauthSupported: true, isActive: false } });
  
  const zoom = await prisma.provider.create({ data: { key: 'zoom', name: 'Zoom', oauthSupported: true } });
  const googleMeet = await prisma.provider.create({ data: { key: 'google_meet', name: 'Google Meet', oauthSupported: true, isActive: false } });
  
  const jira = await prisma.provider.create({ data: { key: 'jira', name: 'Jira', oauthSupported: true } });
  const linear = await prisma.provider.create({ data: { key: 'linear', name: 'Linear', oauthSupported: true, isActive: false } });
  const clickup = await prisma.provider.create({ data: { key: 'clickup', name: 'ClickUp', oauthSupported: true, isActive: false } });
  
  const github = await prisma.provider.create({ data: { key: 'github', name: 'GitHub', oauthSupported: true } });
  const gitlab = await prisma.provider.create({ data: { key: 'gitlab', name: 'GitLab', oauthSupported: true, isActive: false } });
  const bitbucket = await prisma.provider.create({ data: { key: 'bitbucket', name: 'Bitbucket', oauthSupported: true, isActive: false } });
  ...
  await prisma.eyeProvider.createMany({
    data: [
      ...
      { eyeTypeId: meetingEye.id, providerId: googleMeet.id, isAvailableInV1: false },
      { eyeTypeId: meetingEye.id, providerId: msTeams.id, isAvailableInV1: false },
      ...
      { eyeTypeId: codingEye.id, providerId: gitlab.id, isAvailableInV1: false },
      { eyeTypeId: codingEye.id, providerId: bitbucket.id, isAvailableInV1: false },
    ]
  });
  ...
  await prisma.onboardingProgress.create({
    data: {
      ...
      completedSteps: { payment: true, organization_created: true, providers_selected: true },
      isCompleted: true,
      startedAt: new Date(),
